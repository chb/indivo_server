...
VALID_CORE_SCHEMAS = {
    ...
    'immunization': NS+'Immunization',
    'lab': NS+'Lab',
    'medication': NS+'Medication',
    'problem': NS+'Problem',
...
class IndivoSchemaDirUnitTests(InternalTests):
    ...
    def setUp(self):
        ...
        # A valid directory
        self.valid_instance = IndivoSchemaDir(os.path.join(settings.CORE_SCHEMA_DIRS[0], 'medication'))

        # A valid directory with a python transform
        self.valid_instance_py = IndivoSchemaDir(os.path.join(settings.CORE_SCHEMA_DIRS[0], 'allergy'))
    ...
    def test_get_full_schema_path(self):
        schema_path = os.path.join(settings.CORE_SCHEMA_DIRS[0], 'medication/schema.xsd')
        self.assertEqual(self.valid_instance.get_full_schema_path(), schema_path)

        schema_path = os.path.join(settings.CORE_SCHEMA_DIRS[0], 'allergy/schema.xsd')
    ...
    def test_get_full_transform_path(self):
        transform_path = os.path.join(settings.CORE_SCHEMA_DIRS[0], 'medication/transform.xsl')
        self.assertEqual(self.valid_instance.get_full_transform_path(), transform_path)

        transform_path = os.path.join(settings.CORE_SCHEMA_DIRS[0], 'allergy/transform.py')

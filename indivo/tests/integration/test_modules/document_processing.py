...
def test_document_processing(IndivoClient):
  try:
    ...
    user_client.post_app_document(data=data.doc00)
    ...
    doc_id = chrome_client.post_document(data=data.doc00).response[PRD]['Document'][0]
    chrome_client.read_document()
    chrome_client.read_document_meta()

    chrome_client.replace_document(document_id = doc_id, data=data.doc01)
    # SZ: Should return 404, can't replace document of an already replaced document
    chrome_client.replace_document(document_id = doc_id, data=data.doc02)
